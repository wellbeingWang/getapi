/**
 * Global VectorStock Setup
 * Download by http://www.jb51.net
 * Creates all basic functionality for the entire VectorStock website.
 * Built in a [VectorStock] object so as not to clutter up the global
 * variable space.
 * 
...
VectorStock.imagesToLoad = [
	'/img/sprites/elements.png',
	'/img/sprites/repeat.png'
];
 ...
 */
VectorStock.preloadImages = function(imageList) {
...
VectorStock.setupCartPage  = function() {
	this.resetCartButtons()
	this.updateCartTotal();
	// Custom Dropdowns
	VectorStock.newPreviewPane( 'li.browseItem' );
	$('select').dropdown();
	$('.popout').popout();
}

VectorStock.resetCartButtons = function() {
	//$('a.buttoncart').fadeTo('fast', 0.3 ).attr('disabled', 'disabled');
	//$('a.cartconfirm').fadeTo('fast', 1 ).removeAttr('disabled');
}

VectorStock.updateCart = function( items )
{
	var ob  = $('#carttot');
	var msg = 'Cart Updated!';
	
	if( $('nav ul.member').hasClass('hidden') ) {
		$('nav ul.member').fadeIn( 100 );
	}
	
	if( items == 'p' )
	{
		//alert(msg)
		ob.fadeOut( 100 ).fadeIn( 100 ).fadeOut( 100 ).fadeIn( 100 );
		msg = 'Already Purchased';
	}else{
		ob.text( items ).fadeOut( 100 ).fadeIn( 100 ).fadeOut( 100 ).fadeIn( 100 );
	}
	
	if( $('#accountMenu .tabContent').css('display') == 'block' )
	{
		var pos = ob.offset();
		VectorStock.displayMessage( msg, 178, pos.top - 3 );
	}
}

VectorStock.updateGalleryCounter = function( number ){
	$('#gallerytot').text( number ).fadeOut( 100 ).fadeIn( 100 ).fadeOut( 100 ).fadeIn( 100 );
}

VectorStock.updateCartTotal = function(){
	
	var cartTot = 0;
	var credits = parseInt($('#creditsVal var').text());
	$('.browseItem var').each(function() {
		cartTot += parseInt( $(this).text() );
	});
		
	if( credits < cartTot ){
		$("#purchasePrice").parent().addClass('error');
		$('a.cartconfirm').fadeTo("fast",0.3).attr('disabled','disabled');
		//toggle
		if( $( "li.cartMsgDefault" ).css("display") == 'none' ) {
			$("#cartBtnToggle").hide();
			$( "li.cartMsgDefault" ).show();
		}
	}else{
		$("#purchasePrice").parent().removeClass('error');
		$('a.cartconfirm').fadeTo("fast",1).removeAttr('disabled');
		if($( ".cartMsgDefault" ).is(":visible")) $("#cartBtnToggle, .cartMsgDefault").toggle();
	}
	
	$("#purchasePrice").html( cartTot + ' <small>CR</small>' );
	$("#purchaseCartTotal var").text( $('#carttot').text() );
}

/* [setupCommonEvents]
 * Creates common events for things such as press and focus states
 */
VectorStock.setupCommonEvents = function() {
	// Button events
	$('a, input[type=submit]').mousedown(function() { 
		$(this).addClass('press'); 
	});
	$('a, input[type=submit]').mouseup(function() { 
	...
	$(window).resize(function() {
		//VectorStock.resizeCategoryOverlay();
	});
	
	// Any tabs
	$(".tabPanel").delegate('header a', "click", function(e){
		VectorStock.switchTabs(e.target);
		return false;
	});
	
	// Login
	$("#accountTabs").delegate("#signup-btn", "click", function(e){
		VectorStock.logInUser();
	});
...
/* [setupCategoryMenu]
 * Breaks category list into columns and binds open/close events
 */
VectorStock.setupCategoryMenu = function() {
	/* Methods
	 */
	var closeHandler;
	...
	function openCategoryMenu() {
		...
		$('article.main').prepend(overlay);
		VectorStock.resizeCategoryOverlay();
		
		$('#categoryMenu h2 a').text('Hide Categories');
		$('#categoryMenu nav').addClass('open');
		$('#categoryMenu section').slideDown(200, 'easeInOutQuart');
		
		closeHandler = function(e) {
			if ($(e.target).parents('#categoryMenu').length === 0) {
				closeCategoryMenu();
			}
		};
		$('html').bind('mousedown', closeHandler);
	}
	// Closes the category menu
	function closeCategoryMenu() {
		$('article.main > .overlay').remove();
		$('#categoryMenu h2 a').text('Show Categories');
		$('#categoryMenu nav').removeClass('open');
		$('#categoryMenu section').slideUp(120, 'easeOutExpo');
		$('html').unbind('mousedown', closeHandler);
	}
	
	/* Events
	 */
	// Open/Close event
	$('#categoryMenu h2 a').click(function() {
		if ($('#categoryMenu nav').hasClass('open')) {
			closeCategoryMenu();
		} else {
			openCategoryMenu();
		}
...
/* [resizeCategoryOverlay]
 * Adjusts the white category overlay to fit the screen
 */
VectorStock.resizeCategoryOverlay = function() {
	var overlay = $('article.main > .overlay');
	overlay.width($(document).width());
	overlay.height($(document).height());
}


VectorStock.setupSearchPanel = function() {}
